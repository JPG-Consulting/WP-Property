...
= 1.37.0-alpha =
* Complete refactoring JavaScript and CSS registration and enqueuing for full compliance with current WordPress standards.
* Added localization support to all WP-Property scripts via WPP_F::localize_scripts() function.
* Added missing-shortcode catcher for premium features.  If one of the premium feature shortcodes is used when the feature is disabled, or not available, the shortcode will not be rendered in clear text, as handled by WordPress.
* Added [property_attributes] shortcode. Arguments: property_id, list_title, sort_stats_by_groups
* Added [property_taxonomy_terms] shortcode. Arguments: property_id, taxonomy
* Modified JavaScript dependancies: wp-property-global.js is required by front-end and back-end JavaScript styles.
* Global JavaScript wpp object introduced.
* Removed phone number formatting option.
