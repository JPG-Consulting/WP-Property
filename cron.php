 ...
 *
 * @todo This file will need to be removed, but not until our XMLI clients have updated their cron jobs to use new XMLI method. - potanin@UD 6/22/12
 */
...
if($action == 'do_xml_import' && !empty($schedule_hash)) {
  ...
  $sql = "SELECT ID FROM {$wpdb->posts} WHERE post_type = 'attachment' AND post_parent IN (SELECT ID FROM {$wpdb->posts} WHERE post_type = 'property') OR post_excerpt = 'qr_code'";
  foreach($wpdb->get_results($sql) as $row) {
    wp_delete_attachment($row->ID, true);
    print "Deleted attachment: ".$row->ID."\r\n";
  }
  /** Now, delete posts */
  $sql = "SELECT ID FROM {$wpdb->posts} WHERE post_type = 'property'";
  foreach($wpdb->get_results($sql) as $row){
