...
if( !function_exists( 'wpp_featured_properties' ) ) {
  /**
   * Default function to use in template directly
   *
   * @updated 1.37.0 - Moved out of class_widgets.php
   */
  function wpp_featured_properties( $args = false, $custom = false ) {

    if( !is_callable( array( 'FeaturedPropertiesWidget','widget' ) ) ) {
      return;
    }
...
if( !function_exists( 'wpp_search_widget' ) ) {
  ...
  function wpp_search_widget( $args = false, $custom = false ) {
    ...
    if( !is_callable( array( 'SearchPropertiesWidget','widget' ) ) ) {
      return;
    }
...
if(!function_exists('wpp_alternating_row')) {
  ...
  function wpp_alternating_row() {
    ...
    $_even_row = wpp_css( 'global::alternate', 'wpp_even_row', true );
    $_odd_row = wpp_css( 'global::alternate', 'wpp_odd_row', true );

    $wpp_current_row = $wpp_current_row == $_even_row ? $_odd_row : $_even_row;

    return $wpp_current_row;
...
if(!function_exists('get_attribute')) {
  ...
  function get_attribute($attribute = false, $args = '') {
    ...
    switch ($attribute) {
      ...
      case 'property_type':
        $value = $this_property[ 'property_type_label' ];
      break;
    ...
    foreach( $value as $single_value ) {
      $return_values[] = apply_filters( 'wpp_get_attribute', $single_value, array(
      'attribute' => $attribute,
      'args' => $args,
...
if(!function_exists('returned_properties')) {
   ...
   */
   function returned_properties($args = false) {
    ...
    foreach( ( array ) $wpp_query[ 'properties' ][ 'results' ] as $property_id ) {
  ...
  function wpi_draw_pagination($settings = '') {
      ...
      if(!wpp_pagination_history_ran) {
         ...
         */
         jQuery(document).ready(function() {
          ...
          if( typeof jQuery.prototype.address === 'function' ) {
          jQuery.address.change(function(event){
            callPagination(event);
          });
          } else {
            wpp.log( 'jQuery.address() function requested but not available.', 'error' );
          }
    ...
    <div class="properties_pagination <?php echo $settings['class']; ?> wpp_slider_pagination" id="properties_pagination_<?php echo $unique_hash; ?>">
      <div class="wpp_pagination_slider_status">
        ...
        <?php foreach( ( array ) $sortable_attrs as $slug => $label ) { ?>
  ...
  function prepare_property_for_display($property, $args = false) {
    ...
    foreach( $property as $meta_key => $attribute_value ) {
      ...
      if(!is_array($attribute_value)) {
        ...
        $attribute_value = str_replace("\n", "", nl2br(WPP_F::cleanup_extra_whitespace($attribute_value)));

        $attribute_value = apply_filters("wpp_stat_filter_{$meta_key}",$attribute_value, $attribute_scope);

        if( is_callable( $wp_properties[ '_formatting_callback' ][ $attribute_data['type'] ][ 'human' ] ) ) {
          $attribute_value = call_user_func($wp_properties[ '_formatting_callback' ][ $attribute_data['type'] ][ 'human' ], $attribute_value);
        }

        $property[$meta_key] = $attribute_value;
      }
      if (is_array($attribute_value)){
        if( is_callable( $wp_properties[ '_formatting_callback' ][ $attribute_data['type'] ][ 'human' ] ) ) {
          array_map($wp_properties[ '_formatting_callback' ][ $attribute_data['type'] ][ 'human' ],$attribute_value);
        }
      }
    }
...
if(!function_exists('get_features')) {
  function get_features($args = '', $property = false) {
    ...
    $property = $property ? ( object ) $property : $post;
    ...
    foreach ( array_filter( ( array ) get_the_terms( $args[ 'property_id' ], $args[ 'type' ] ) ) as $feature ) {
      $features_html[] = $args[ 'links' ] ? '<a href="' . get_term_link( $feature ) . '">' . $feature->name . '</a>' : $feature->name;
    }

    $features_html = array_filter( ( array ) $features_html );
    ...
    switch( $args[ 'format' ] ) {
      ...
      case 'list':
        $html[] = "<li>" . implode( $features_html, "</li><li>" ) . "</li>";
      break;
...
if( !function_exists( 'draw_stats' ) ) {
   ...
   * @since 1.11
   */
  function draw_stats( $args = false, $_property = false ) {
    global $wp_properties, $property;

    $_property = $_property ? (object) $_property : (object) $property;

    $_property = prepare_property_for_display( $_property );

    $args = array_filter( ( array ) wp_parse_args( $args, array(
      ...
      'make_link' => 'true',
      'hide_false' => 'false',
      'display_detailed' => 'false', //** false|true|only */
      'return' => false,
      'container_tag'  => 'div'
    )));
    ...
    $property_groups = $wp_properties[ 'property_groups' ]; /* group key => group data */
    $property_stats_groups = $wp_properties[ 'property_stats_groups' ]; /* attribute key => group key */

    if( $args[ 'sort_by_groups' ]=='true' && is_array( $property_groups ) && empty( $args[ 'group' ] ) ) {
      $_grouped = true;
      $title = true;
    }

    //** Prepare attributes */
    foreach( ( array ) WPP_F::get_stat_values_and_labels( $_property, $args ) as $attribute_label => $value ) {

      $attribute_key = $labels_to_keys[ $attribute_label ];
      $attribute_data = WPP_F::get_attribute_data( $attribute_key );

      //** Determine how we should to display the fields with type 'Property Meta' */
      if (
        ($display_detailed=='only' && $attribute_data['type']!='detail') ||
        (($display_detailed=='false' || $display_detailed==false) && $attribute_data['type']=='detail')
      ){
        continue;
      }

      //** Do not show attributes that have value of 'value' if enabled */
      if( ($hide_false == 'true' || $show_true_as_image=='true') && ($value == 'false' || empty($value) || ($attribute_data[ 'type' ] == 'boolean' && $value == __( 'No', 'wpp' )) ) ) {
        continue;
      }

      //* Determine if the current attribute is address and display_address is set */
      //* should be moved to _formatting_callback functionality *
      if( $attribute_key == $wp_properties[ 'configuration' ][ 'address_attribute' ] && !empty( $_property->display_address ) ) {
        $value = $_property->display_address;
      }
      //*/

      if( $attribute_data[ 'type' ] == 'boolean' ){
        if ( $value == __( 'Yes', 'wpp' ) ) {
          $value = ($show_true_as_image == 'true') ? '<div class="true-checkbox-image"></div>' : $value;
          $attribute_data['ui_class'] .= ' show_true_as_image';
        }
      }

      if (!$_grouped) {
        $attribute_data[ 'group_label' ] = 0;
        $attribute_data[ 'group_key' ] = '';
      }

      $_stats[ $attribute_data[ 'group_label' ] ][ 'group_key' ] = $attribute_data[ 'group_key' ];
      $_stats[ $attribute_data[ 'group_label' ] ][ 'attributes' ][ $attribute_key ] = $attribute_data;
      $_stats[ $attribute_data[ 'group_label' ] ][ 'attributes' ][ $attribute_key ][ 'label' ] = $attribute_label;
      $_stats[ $attribute_data[ 'group_label' ] ][ 'attributes' ][ $attribute_key ][ 'values' ][] = $value;

      $_basic_stats[ $attribute_label ] = $value;
    ...
    foreach( ( array ) $_stats as $_group_label => $_group_data ) {

      foreach( ( array ) $_group_data[ 'attributes' ] as $attribute_key => $attr_data ) {

        $label = $attr_data[ 'label' ];
        $value = $attr_data[ 'values' ][0];

        switch($display) {

          case 'list':
            $_items[ $_group_label ][] = '<li class="' . wpp_css( 'attribute_list::list_item', 'wpp_attribute '.$attr_data['ui_class'], true ) .  ' ' . wpp_alternating_row() . '" data-attribute_key="' . $attribute_key . '">';
            $_items[ $_group_label ][] = '<span class="' . wpp_css( 'attribute_list::attribute', 'attribute', true ) . '">' . $label . '<span class="separator">:</span></span>';
            $_items[ $_group_label ][] = '<span class="' . wpp_css( 'attribute_list::attribute', 'value', true ) . '">' . $value . '</span>';
            $_items[ $_group_label ][] = '</li>';
          break;

          case 'dl_list':
            $_items[ $_group_label ][] = '<dt class="'. wpp_css( 'attribute_list::dl_list_item', 'wpp_stat_dt_' . $attribute_key . $attr_data['ui_class'], true ) . '" data-attribute_key="' . $attribute_key . '">' .  $label . '</dt>';
            $_items[ $_group_label ][] = '<dd class="'. wpp_css( 'attribute_list::dl_list_item', 'wpp_stat_dd_' . $attribute_key . ' ' . $alt . ' ' .$attr_data['ui_class'], true ) . '" data-attribute_key="' . $attribute_key . '">' . $value . '</dd>';
            break;

          case 'plain_list':
            $_items[ $_group_label ][] = '<span class="' . wpp_css( 'attribute::span_label', 'wpp_attribute '.$attr_data['ui_class'], true ) .  '" data-attribute_key="' . $attribute_key . '">' .  $label . '<span class="separator">:</span></span>';
            $_items[ $_group_label ][] = '<span class="value" data-attribute_key="' . $attribute_key . '">' .  $value . '</span>';
            $_items[ $_group_label ][] = '<br />';
            break;

          case (preg_match('~^([^_]+)\_([^_]+)\_(.*?)$~im',$display,$tags) && $tags[0]==$display):

            $_items[ $_group_label ][] = '<'.$tags[1].' class="' . wpp_css( 'attribute_list::'.$display, 'wpp_attribute '.$attr_data['ui_class'], true ) .  ' ' . wpp_alternating_row() . '" data-attribute_key="' . $attribute_key . '">';
            $_items[ $_group_label ][] = '<'.$tags[2].' class="' . wpp_css( 'attribute_list::attribute', 'attribute', true ) . '">' . $label . '</'.$tags[2].'>';
            $_items[ $_group_label ][] = '<'.$tags[3].' class="' . wpp_css( 'attribute_list::attribute', 'value', true ) . '">' . $value . '</'.$tags[3].'>';
            $_items[ $_group_label ][] = '</'.$tags[1].'>';
          break;
    ...
    $ouput[] = '<'.$container_tag.' class="' . wpp_css( 'attribute_list::list_item', 'wpp_attributes', true ) . '">';

    foreach( ( array ) $_items as $_group_label => $html ) {

      if( $title ) {
        $ouput[] = '<h2 class="' . wpp_css( 'attribute_list::list_item', 'wpp_stats_group', true ) . '">' . $_group_label . '</h2>';
      }

        switch($display) {

          case 'list':
            $ouput[] = '<ul class="' .  wpp_css( 'attribute_list::list_item', 'wpp_property_stats overview_stats '.$display, true ) . '">' . implode( '', ( array ) $html ) . '</ul>';
            break;

          case 'dl_list':
          $ouput[] = '<dl class="' .  wpp_css( 'attribute_list::list_item', 'wpp_property_stats overview_stats '.$display, true ) . '"> ' . implode( '', ( array ) $html ) . '</dl>';
            break;

          case 'plain_list':
          $ouput[] = implode( '', ( array ) $html );
            break;
    ...
    $ouput[] = '</'.$container_tag.'>'; /* .wpp_attributes */

    if( $args[ 'return' ] ) {
      return implode( '', ( array ) $ouput );
    }

    echo implode( '', ( array ) $ouput );
 ...
 */
  function sort_stats_by_groups($stats = false, $args = '') {
    ...
    if( !is_array( $property_groups ) || !is_array( $stats_groups ) ) {
      return;
    }
    ...
    if($args['fix_stats_array'] = true) {
      foreach( $stats as $meta_key => $values ) {

        //** If multiple values allowed, always use array */
        if( $args[ 'allow_multiple_values' ] && !is_array( $values ) ) {
          $values = array( $values );
        }
        ...
        $fixed_stats[$attribute_label] = $values;
      }
      $stats = $fixed_stats;
    }
    ...
    foreach( ( array ) $stats as $label => $value ) {
      ...
      if( $g_slug && !key_exists( $g_slug, $property_groups ) ) {
        $g_slug = false;
      }
       if($args['includes_values'] == true) {
    ...
    foreach( ( array ) $filtered_stats as $key => $data ) {
      if(empty($data)) {
...
if( !function_exists( 'draw_property_search_form' ) ) {
  ...
  function draw_property_search_form($args = false) {
    ...
    WPP_F::wp_enqueue_script( 'jquery-number-format' );
    ...
    if(array_key_exists('property_type', array_fill_keys($search_attributes, 1)) && is_array($searchable_property_types) && count($searchable_property_types) > 1 ) {
      ...
      if(!empty($wp_properties['property_types'])) {
        foreach( ( array ) $wp_properties[ 'property_types' ] as $key => $value ) {
    ...
    if(is_array($searchable_property_types) && !array_key_exists('property_type', array_fill_keys($search_attributes, 1))) {
      foreach( ( array ) $searchable_property_types as $property ) {
    ...
    $count = 0;
    foreach( ( array ) $search_groups as $group_key => $search_attributes ) {
      $count++;
      ...
      <?php if($is_a_group) { ?>
      <span class="wpp_search_group_title wpp_group_<?php echo $this_group; ?>_title"><?php echo $property_groups[$this_group][ 'name' ]; ?></span>
      ...
      foreach( ( array ) $search_attributes as $attrib ) {
...
if( !function_exists( 'wpp_render_search_input' ) ) {
  ...
  function wpp_render_search_input($args = false) {
    ...
    extract( wp_parse_args( $args, $defaults = array(
      'type' => 'input',
      'input_type' => false,
      'search_values' => false,
      'attrib' => false,
    ...
    switch($use_input_type) {
      ...
      case 'date_picker':
        ...
        ?>
        <input id="<?php echo $random_element_id; ?>"  class="<?php echo $attribute_data[ 'ui_class' ]; ?> wpp_date_picker" name="wpp_search[<?php echo $attrib; ?>]" value="<?php echo $value; ?>" type="text" />
        <script>
          jQuery(document).ready(function(){
      ...
      case 'range_dropdown':
        ...
        <?php foreach( ( array ) $grouped_values as $v ) : ?>
      ...
      case 'multi_checkbox':
        ...
        <?php foreach( ( array ) $search_values[$attrib] as $value_label ) : ?>
      ...
      <?php else : ?>
        ...
        <?php foreach( ( array ) $search_values[$attrib] as $key => $v ) : ?>
...
if( !function_exists( 'wpp_get_image_link' ) ) {
   ...
   *
   */
  function wpp_get_image_link( $attachment_id = false, $size = false , $args = array() ) {
    return WPP_F::get_image_link( $attachment_id, $size, array_merge( array( 'cache_group' => 'wpp_image_link' ), $args ) );
    }
      }
  ...
  function wpp_inquiry_form( $args = array(), $post_id = null ) {
    ...
    if(!$inquiry) {
      ...
      $defaults = array(
        ...
        'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun' ) . '</label><textarea id="comment" name="comment" rows="8" aria-required="true"></textarea></p>',
        ...
        <div id="respond">
          ...
          <?php else : ?>
            <form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
              ...
              <p class="form-submit">
                <input name="submit" type="submit" id="<?php echo esc_attr( $args[ 'id_submit' ] ); ?>" value="<?php echo esc_attr( $args[ 'label_submit' ] ); ?>" class="<?php wpp_css( 'global::buttons', 'wpp_button' ); ?>" />
