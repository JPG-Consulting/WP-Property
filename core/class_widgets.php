...
class Property_Attributes_Widget extends WP_Widget {
   ...
   *
   * @todo This should reference the [property_attributes] shortcode for consistency. - potanin@UD
   * @since 1.31.0
   */
  function widget($args, $instance) {
    ...
    foreach($instance as $slug => $option) {
      ...
      $values = get_attribute($slug, array('return' => 'true'));

      //** If an array is returned, we concatenate our values using the separator */
      if(is_array($values)) {
        foreach($values as $count => $single_value) {

          $single_value = trim($single_value);

          $single_value = html_entity_decode($single_value);

          $single_value = nl2br(WPP_F::cleanup_extra_whitespace($single_value));

          $single_value = do_shortcode($single_value);

          if(!empty($single_value)) {

            if(WPP_F::isURL($single_value)) {
              $single_value = '<a class="wpp_link" href="'. $single_value .'">' . $single_value. '</a>';
            }

            $values[$count] = '<span class="value">' . $single_value . '</span>';
      ...
      $attribute_slugs[] = $slug;

      $attributes_html[] = '<li class="'. $slug . ' wpp_attribute_list_item">' . ($show_labels ? '<span class="attribute">' . $attribute['label'] . '<span class="separator">:</span> </span>' : '') . '</span>' . $values . '</li>';
    }


    if(!empty($attributes_html)) {
      $html['attributes_html'] = '<ul class="wpp_widget_attribute_list" wpp_attributes="' . implode(' ', $attribute_slugs) . '">' . implode('', $attributes_html) . '</ul>';
    }

    $html[] = $after_widget;

    if(!empty($html['attributes_html'])) {
      echo implode('', $html);
    } else {
      return false;
    }
...
/**
 * Other "Sibling" Properties Widget
 *
 */
class OtherPropertiesWidget extends WP_Widget {
  ...
  function OtherPropertiesWidget() {
    ...
    parent::__construct(
      false,
      sprintf(__('Other %1$s','wpp'), ucfirst(WPP_F::property_label('plural'))),
      array(
        'description' => sprintf(__('Display a list of %2$s that share a parent with the currently displayed %1$s.', 'wpp'), ucfirst(WPP_F::property_label('singular')), ucfirst(WPP_F::property_label('plural')))
      ),
      array(
        'width' => 300
      )
    ...
    foreach($properties as $single) {
      ...
      $image = wpp_get_image_link($this_property->featured_image, $image_type, array('return'=>'array'));
        ...
        <?php if ($instance['enable_more'] =='on') {
          echo '<p class="more"><a href="'. $this_property->permalink . '" class="' . wpp_css( 'global::buttons', 'wpp_button', true ) . '">' . __( 'More','wpp') . '</a></p>';
        } ?>
      ...
      if ($instance['enable_view_all'] =='on') {
        $html[] = '<p class="view-all"><a href="'. site_url() .'/'. $wp_properties['configuration']['base_slug'] .'" class="' . wpp_css( 'global::buttons', 'wpp_button', true ) . '">'.__('View All','wpp').'</a></p>';
      }
  ...
  function form($instance) {
    ...
    <p><?php echo sprintf(__('The widget will show other %2$s that share a parent with the currently displayed %1$s.','wpp'), ucfirst( WPP_F::property_label( 'singular' )), ucfirst( WPP_F::property_label( 'plural' ))); ?></p>

    <p>
      <label for="<?php print($this->get_field_id('title')); ?>"><?php _e('Title:','wpp'); ?></label>
    ...
    <ul>
      ...
      <li>
        ...
        <label for="<?php echo $this->get_field_id('show_properties_of_type'); ?>"><?php echo sprintf(__('If %1$s has no parent, show other %2$s of same type.','wpp'), ucfirst( WPP_F::property_label( 'singular' ) ), ucfirst( WPP_F::property_label( 'plural' )) ); ?></label>
      </li>
      <li>
        ...
        <label for="<?php echo $this->get_field_id('shuffle_order'); ?>"><?php echo sprintf(__('Randomize order of displayed %1$s.','wpp'), ucfirst( WPP_F::property_label( 'plural' )) ); ?></label>
      </li>
    </ul>
...
class ChildPropertiesWidget extends WP_Widget {
  ...
  function ChildPropertiesWidget() {
    parent::WP_Widget(false, $name = sprintf(__('Child %1s','wpp'), WPP_F::property_label('plural')), array('description' => sprintf(__('Show child %2$s (if any) for currently displayed %1$s', 'wpp'),WPP_F::property_label('singular'),WPP_F::property_label('plural'))));
  }
  ...
  function widget($args, $instance) {
      ...
      <div class="property_widget_block apartment_entry clearfix" style="<?php echo ($width ? 'width: ' . ($width+5) . 'px;' : ''); ?>">
        ...
        <?php if(is_array($stats)): ?>
          <?php foreach($stats as $stat): ?>
            <?php $content = nl2br(WPP_F::cleanup_extra_whitespace(apply_filters("wpp_stat_filter_{$stat}", $this_property->$stat))); ?>
        ...
        <?php if ($instance['enable_more'] =='on') : ?>
          <p class="more"><a href="<?php echo $this_property->permalink; ?>" class="<?php wpp_css( 'global::buttons', 'wpp_button' ); ?>"><?php _e('More','wpp'); ?></a></p>
        <?php endif; ?>
      </div>
      <?php
      unset($this_property);
    ...
    if ($instance['enable_view_all'] =='on') {
      echo '<p class="view-all"><a href="'. site_url() .'/'. $wp_properties['configuration']['base_slug'] .'" class="' . wpp_css( 'global::buttons', 'wpp_button' ) . '">'.__('View All','wpp').'</a></p>';
    }
  ...
  $('input.check_me_child').change(function(){
    if($(this).attr('checked') !== true){
      ...
      $('p#choose_thumb_child').css('display','none');
                 ...
                 <p><?php echo sprintf(__('The widget will not be displayed if the currently viewed %1$s has no children.','wpp'), WPP_F::property_label('singular')); ?></p>
            <p>
...
class FeaturedPropertiesWidget extends WP_Widget {
  ...
  function FeaturedPropertiesWidget() {
    parent::WP_Widget(false, $name =  sprintf(__('Featured %1s','wpp'), WPP_F::property_label('plural')), array('description' => sprintf(__('List of %1$s that were marked as Featured', 'wpp'), WPP_F::property_label('plural'))));
  }
  ...
  function widget($args, $instance) {
    ...
    if(!isset($property_stats['property_type'])) {
      $property_stats['property_type'] = sprintf(__('%1$s Type', 'wpp'), ucfirst(WPP_F::property_label('singular')));
    }
      ...
      <ul class="wpp_widget_attribute_list">
        <?php if(is_array($stats)): ?>
          <?php foreach($stats as $stat):
            ...
            $content = nl2br(WPP_F::cleanup_extra_whitespace(apply_filters("wpp_stat_filter_{$pstat}", $this_property->$pstat)));
    ...
    if ($instance['enable_view_all'] =='on') {
      echo '<p class="view-all"><a href="'. site_url() .'/'. $wp_properties['configuration']['base_slug'] .'" class="' . wpp_css( 'global::buttons', 'wpp_button', true ) . '">' . __('View All','wpp') . '</a></p>';
    }
    echo '<div class="clear"></div>';
    echo '</div>';
    echo $after_widget;
  ...
  function form($instance) {
    ...
    if(!isset($property_stats['property_type'])) {
      $property_stats['property_type'] = sprintf(__('%1$s Type', 'wpp'), WPP_F::property_label('singular'));
    }
    ?>
    <script type="text/javascript">
      //hide and show dropdown whith thumb settings
...
class LatestPropertiesWidget extends WP_Widget {
  ...
  function LatestPropertiesWidget() {
    parent::WP_Widget(false, $name =  sprintf(__('Latest %1$s','wpp'), WPP_F::property_label('plural')), array('description' => sprintf(__('List of the latest %1$s created on this site', 'wpp'), WPP_F::property_label('plural'))));
  }
  ...
  function form($instance) {
    ...
    <p>
      <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','wpp'); ?>
        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo (!empty($title)) ? $title : sprintf(__('Latest %1$s', 'wpp'), ucfirst(WPP_F::property_label('plural'))); ?>" />
      </label>
    </p>
    <p>
      <label for="<?php echo $this->get_field_id('hide_image'); ?>">
 ...
 class SearchPropertiesWidget extends WP_Widget {
     ...
     */
     function form($instance) {
      ...
      if($error) {
        echo '<p>' . sprintf(__('No searchable %1$s types were found.','wpp'), WPP_F::property_label('singular')) . '</p>';
        return;
      }
     ...
     <script type="text/javascript">
      ...
      jQuery(document).ready(function(){
        ...
        if( typeof jQuery.prototype.sortable === 'function' ) {
    ...
    <ul id="wpp_property_search_wrapper_<?php echo $this->number; ?>" class="wpp_property_search_wrapper">
      ...
      <li class="wpp_property_types">
        <p><?php echo sprintf(__('%1$s types to search:','wpp'), ucfirst(WPP_F::property_label('singular'))); ?></p>
        <ul>
  ...
  class GalleryPropertiesWidget extends WP_Widget {
    ...
    function GalleryPropertiesWidget() {
      parent::WP_Widget(false, $name =  sprintf(__('%1s Gallery','wpp'), WPP_F::property_label()), array('description' => sprintf(__('List of all images attached to the current %1$s', 'wpp'), WPP_F::property_label('singular'))));
    }
