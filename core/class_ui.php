...
class WPP_UI {
  ...
  static function get_ui( $args = false ) {
    ...
    //** Defaults */
    $defaults = array( 'class' => self );

    //** Prepare args */
    $args = wp_parse_args( $args, $defaults);

    //** If we request core settings page */
    if ( $args[ 'class' ] == self ) {

      //** Check template file for existance */
      if ( file_exists( WPP_UI . '/settings/'.$args[ 'ui' ].'.php' ) ) {

        //** Buffer page content */
        ob_start();
        include_once WPP_UI . '/settings/'.$args[ 'ui' ].'.php';
        $html = ob_get_clean();
    ...
    //** If we request settings page from another class (PF) */
    } else {

      //** Check if callback function exists */
      if( !is_callable( array( $args[ 'class' ], $args[ 'ui' ] ) ) ) {

        //** Return error if callback function doesn't exist */
        return array(
          'success' => false,
          'message' => sprintf( __( 'Could not find function "%s::%s" for UI section.', 'wpp' ), $args[ 'class' ], $args[ 'ui' ] )
        );

      } else {

        //** Call callback function to get page content */
        ob_start();
        call_user_func( array( $args[ 'class' ], $args[ 'ui' ] ) );
        $html = ob_get_clean();
      }
    }

    //** Return what we got */
    return array( 'success' => true, 'ui' => $html, 'id' => (string)$args[ 'class' ].'_'.$args[ 'ui' ] );
  ...
  static function main_settings() {
    ...
    ob_start();
    ?><div style="display:none;" class="wpp_settings_section wpp_section_self_main" data-menu-item-ui="main" data-menu-item-class="self"><?php
    include_once WPP_UI . '/settings/main.php';
    ?></div><?php
    return ob_get_clean();
  ...
  */
    ...
    <table class="widefat property_meta">
        ...
        <?php else : ?>
          <tr class="wpp_attribute_row_parent wpp_attribute_row <?php if (is_array($wp_properties['hidden_attributes'][$property['property_type']]) && in_array('parent', $wp_properties['hidden_attributes'][$property['property_type']])) {  echo 'disabled_row;'; } ?>">
            ...
            <td>
              ...
              <span class="description"><?php echo sprintf(__('ID of parent %1$s', 'wpp'), WPP_F::property_label( 'singular' ) ); ?></span>
            </td>
          </tr>
        <?php endif; ?>
      ...
      foreach ($property_stats as $slug => $label) {
        ...
        if(in_array($slug, $upwards_inherited_attributes)) {
          ...
          $attribute_description = array(sprintf(__('Values aggregated from child %1$s.','wpp'), WPP_F::property_label( 'plural' )));
        }
        ...
        <tr class="<?php echo implode(' ', $row_classes); ?>">
          ...
          <td class="wpp_attribute_cell">
            ...
            foreach($values as $value_count => $value) {
              ...
              //** Fix value for boolean values */
              //if ($value === true || ($input_type == 'checkbox' && $value == 1)) {
              if($input_type == 'checkbox' && ($value == 1 || $value === true)) {
                $value = 'true';
              }
                ...
                switch ($input_type) {
                  ...
                  case 'dropdown':
                    foreach ($predefined_values as $option) {
                     ...
                     $html_input = "<select row_count='{$value_count}' id='wpp_meta_{$slug}' name='wpp_data[meta][{$slug}][{$value_count}]' class='{$attribute_data['ui_class']}'><option value=''> - </option>" . implode($predefined_options[$slug]) . "</select>";
                  break;
              ...
              echo '<li class="wpp_field_wrapper">' . apply_filters("wpp_property_stats_input_$slug", $html_input, $slug, $property, $attribute_data) . '</li>';
      ...
      //* 'Property Meta' fields should be shown only in 'General Information' metabox */
      //** We've got rid of property_meta */
      /*if($instance == 'wpp_property_meta') {
        ...
        foreach ($property_meta as $slug => $label) {
          ...
          <tr class="<?php echo implode(' ', $row_classes); ?>">
            ...
            <td>
              <ul class="wpp_single_attribute_entry_list">
                <?php foreach( $values as $value_count => $single_value) { ?>
                <li class="wpp_field_wrapper">
                  <span class="disabled_message"><?php echo sprintf(__('Editing %s is disabled, it may be inherited.', 'wpp'), $label); ?></span>
                  <textarea class="text-input" name="wpp_data[meta][<?php echo $slug; ?>][<?php echo $value_count; ?>]"><?php echo $single_value; ?></textarea>
                </li>
                <?php } ?>
              </ul>
  ...
  function metabox_property_filter($wp_list_table) {
    ...
    global $wp_properties, $multiple_filters;
    //** Be sure we have array in $multiple_filters */
    $multiple_filters = !empty( $multiple_filters ) ? $multiple_filters : array();
    ?>
    <div class="misc-pub-section">
      ...
      if (!empty($filters)) {
        foreach ($filters as $key => $filter) {
          ...
          if ( !empty( $filter['values'] ) && ( count( $filter['values'] ) > 1 || in_array($key, $multiple_filters) ) ) {
            ?>
  ...
  function property_type_selector($property) {
    ...
    <?php } else { ?>
      <input type="hidden" id="wpp_meta_property_type" name="wpp_data[meta][property_type]" id="property_type" value="<?php echo ($property['property_type'] ? strtolower($property['property_type']) : $property_type_slugs[0]); ?>" />
    <?php }
