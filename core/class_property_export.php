...
class wpi_property_export {
  ...
  ?>
    <div class="wpp_settings_block">
      ...
      <ul style="margin: 15px 0 0 10px">
        ...
        <li><b>property_type</b> - string - <?php sprintf(_e('Slug for the %1$s type.', 'wpp'), WPP_F::property_label('singular')); ?></li>
  ...
  function get_property_export_url(){
    if($apikey = WPP_F::get_api_key()){
      if(empty($apikey)) return __("There has been an error retrieving your API key.", "wpp");
      // We have the API key, we need to build the url
      return admin_url('admin-ajax.php')."?action=wpp_export_properties&api=".$apikey;
    }
    //return __("There has been an error retrieving your API key.", "wpp");
    return false;
  }
  ...
  $taxonomies = $wp_properties['taxonomies'];
    ...
    if(count($results) == 0) {
      die(sprintf(__('No published %1$s.', 'wpp'), WPP_F::property_label('plural')));
    }
