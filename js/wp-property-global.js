/**
 * WP-Property Global JavaScript
 *
 * This file is included on all back-end and front-end pages.
 *
 * Depending on template, the wpp object may already be declared and
 * loaded with WP-Property configuration variables.
 *
 * Other scripts should exend the global wpp object.
 *
 */
 ...
 /* Declare global var if it not setup already */
  var wpp = jQuery.extend( true, {
     ...
     *
     * Basic Usage:
     * - wpp.log( 'I need a shower.' );
     * - wpp.log( 'There is no water.', 'error' );
     * - wpp.log( { some: 'information' } );
     *
     * Advanced Usage:
     * - wpp.log( 'As long as the second variable is not a string, all additional variables will be added to the log:', { one: 'test', two: 'test' }, { puppies: 'yes', kittens : 'no' }, 7, 2 );
     * - wpp.log( cat === big , 'If we have a small cat, this message will not be displayed at all. When first variable is a boolean, it must be true for rest of variables to be displayed..' );
     *
     *
     * @author potanin@UD
     */
    log: function ( notice, type, override ) {
      ...
      if( !window.console || ( !override && ( !wpp.developer_mode || !window.console ) ) || typeof notice === 'boolean' && !notice ) { return false; }

      wpp.log.console = {
        log: function( args ) {
          if( args.type == 'info' ) { return wpp.log.console.info( args ); }
          if( args.type == 'error' ) { return wpp.log.console.error( args ); }
          if( args.type == 'dir' ) { return wpp.log.console.error( args ); }
          return ( wpp.console_options && wpp.console_options.show_log ) ? console.log.apply( console, args.items ) : false;
        },
      ...
      return wpp.log.console.log({
        items: [ 'wpp::' ].concat( jQuery.makeArray( typeof notice === 'string' && typeof type === 'object' ? arguments : [ notice ] ) ),
    ...
    render_map: function( selector, args ) {
      ...
      if( !jQuery( selector ).length ) {
        return wpp.log( l10n.maps_failure, 'error' );
      }
      ...
      args = jQuery.extend( true, {
        ...
        infowindow: {
          title: l10n.infobox_title,
          content: l10n.infobox_content,
     ...
     * @author peshkov@UD
     */
    get_service: function( service, get, args ) {
      'use strict'; wpp.log( 'wpp.get_service()', arguments );

      wpp.ajax( 'get_api_service', { service: service, get: get, args: args }, function( response ) {
     ...
     * WPP AJAX Handler
     *
     * Example: wpp.ajax( 'some_action', { arg: value }, function() {  } );
     *
     * @todo Need to automatically save schedule data first.
     */
    ajax: function( wpp_action, ajax_args, callback, args ) {
      //'use strict'; wpp.log( 'wpp.ajax()' , arguments );
      ...
      var response = function( jqXHR ) {
        'use strict'; wpp.log( 'wpp.ajax.response()', arguments );
        ...
        try {
          ...
          if( typeof result.response_text !== 'object' || jqXHR.responseText === '' ) {
            throw new Error( wpp.strings.ajax_response_empty );
          }

          if( jqXHR.status === 500 ) {
            throw new Error( wpp.strings.internal_server_error );
          }
        ...
        if( jqXHR.statusText === 'timeout' ) {
          result.status = 408;
          result.response_text = result.response_text ? result.response_text : wpp.strings.server_timeout;
        }
      ...
      return jQuery.ajax( args = jQuery.extend( true, {
        url: ajaxurl + '?action=wpp_' + wpp_action,
        data: jQuery.extend( true, { args: ajax_args }, typeof data === 'object' ? data : {} ),
        dataType: 'json',
        type: 'POST',
        timeout: ( ( typeof wpp.server === 'object' ? wpp.server.max_execution_time : 30 ) - 10 ) * 1000,
        beforeSend: function ( xhr ) {
          jQuery( document ).trigger( 'wpp.ajax.beforeSend' );
    ...
    table: {
      ...
      add_row: function( element ) {
        'use strict'; wpp.log( 'wpp.add_row()', arguments );
        ...
        jQuery( ".wpp_delete_row .wpp_link", added_row ).show();
      ...
      remove_row: function( element ) {
        ...
        //* Confirm */
        if ( !confirm( l10n.remove_confirmation ) ) {
          return false;
        }
        //* Blank out all values */
